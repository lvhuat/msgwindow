...
// Window 是一个消息并行处理的缓冲区窗口，它是基于环形缓冲区，又读取位，写入位，已经确认集合，
...
// Window 通过一个滑动窗口，解决的是有序序列并行处理消息吞吐量的问题
type Window struct {
	ring     []int64 // 令牌环
	ringLen  int64   // 环长度
...
func New(l int64, nextPos int64) *Window {
	if (l & (l - 1)) != 0 {
...
func (window *Window) left() int64 {
	return atomic.LoadInt64(&window.wPos) - atomic.LoadInt64(&window.rPos)
}

func (window *Window) wait(condition func() bool, do func()) {
	window.mutex.Lock()
...
func (window *Window) notifyAll() {
	window.cond.Broadcast()
}

// Write 向窗口写入新的offset，如果长度已经满了，则会block
// 不能使用并行访问
func (window *Window) Write(offset int64) {
...
func (window *Window) Confirm(offset int64) {
	wPos := atomic.LoadInt64(&window.wPos)
	rPos := atomic.LoadInt64(&window.rPos)
...
func (window *Window) ReadPos() int64 {
	return atomic.LoadInt64(&window.rPos)
}

func (window *Window) WritePos() int64 {
	return atomic.LoadInt64(&window.wPos)
}

// Read 读取已经confirm的消息
func (window *Window) TryRead() (int64, bool) {
	// 如果长度为0，则继续等待
	for window.left() == 0 {
		return -1, false
	}
